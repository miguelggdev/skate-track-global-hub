...
def test_list_denies_non_privileged(api_client):
    ...
    with patch("api.routes.automations._fetch_app_role_async", new=AsyncMock(return_value="athlete")):
...
def test_list_allows_admin(api_client, mock_supabase):
    ...
    with patch("api.routes.automations._fetch_app_role_async", new=AsyncMock(return_value="admin")), \
...
def test_patch_denies_non_admin(api_client):
    ...
    with patch("api.routes.automations._fetch_app_role_async", new=AsyncMock(return_value="leader")):
