...
import { usePaginatedTransactions, useFinancialStats, useUpdateTransaction, useIncomeDistribution } from '@/hooks/useTransactions';
import { useInvoices, useInvoiceSummary, useMarkInvoicePaid, useUpcomingInvoices } from '@/hooks/useInvoices';
...
const Finance = () => {
  ...
  const { data: incomeDistribution = [] } = useIncomeDistribution();
  const { data: upcomingInvoices = [], isLoading: upcomingInvoicesLoading } = useUpcomingInvoices();
      ...
      <div className="space-y-6">
        ...
        <Tabs defaultValue="overview" className="space-y-6">
          ...
          <TabsContent value="overview" className="space-y-6">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>Facturas pendientes de pago, ordenadas por fecha</CardDescription>
              </CardHeader>
              <CardContent>
                {upcomingInvoicesLoading ? (
                      ...
                      <div key={invoice.id} className="flex items-center justify-between p-3 border rounded-lg">
                        <div>
                          <p className="font-medium">
                            {invoice.concept} — {invoice.athletes
                              ? `${invoice.athletes.first_name} ${invoice.athletes.last_name}`
          ...
          <TabsContent value="budgets" className="space-y-6">
            ...
            <CardContent>
              <div className="space-y-6">
                <div className="rounded-lg border border-amber-200 bg-amber-50 dark:bg-amber-950/20 dark:border-amber-800 px-4 py-3 flex items-center gap-2">
                  <span className="text-amber-600 dark:text-amber-400 text-sm font-medium">
                    Datos de ejemplo — Esta sección mostrará presupuesto real cuando se configure
                  </span>
                </div>
