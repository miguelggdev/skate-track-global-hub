...
const RecentActivity = () => {
  const { data: recentActivity = [], isLoading: loading } = useQuery({
    ...
    queryFn: async (): Promise<Activity[]> => {
      ...
      if (athletesRes.error) throw new Error((athletesRes.error as { message: string }).message);
      ...
      <CardContent className="space-y-4 px-6">
        {recentActivity.length === 0 ? (
          ...
          recentActivity.map((activity) => (
            <div key={`${activity.athlete_name}-${activity.rawDate.getTime()}`} className="flex items-center justify-between">
              <div className="flex items-center space-x-3 min-w-0 flex-1">
                ...
                <div className="min-w-0 flex-1">
                  <p className="font-medium text-foreground text-sm truncate">{activity.athlete_name}</p>
