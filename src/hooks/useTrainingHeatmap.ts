import React from 'react';
import { useQuery } from '@tanstack/react-query';
import { supabase } from '@/integrations/supabase/client';
import { startOfWeek, endOfWeek, startOfMonth, endOfMonth, format, parseISO } from 'date-fns';
import { es } from 'date-fns/locale';

interface HeatmapData {
  day: string;
  dayIndex: number;
  hour: number;
  intensity: number; // 0-100 based on session count and attendance
  sessionCount: number;
  totalAttendance: number;
  averageAttendance: number;
}

interface TrainingSession {
  id: string;
  date: string;
  start_time: string;
  end_time: string;
  training_type: string;
  name: string;
  max_participants?: number;
  attendanceCount?: number;
}

export const useTrainingHeatmap = (viewMode: 'week' | 'month' = 'week') => {
  const { data, isLoading, error } = useQuery({
    queryKey: ['training-heatmap', viewMode],
    queryFn: async () => {
      const now = new Date();
      let startDate: Date;
      let endDate: Date;

      if (viewMode === 'week') {
        startDate = startOfWeek(now, { weekStartsOn: 1 }); // Monday
        endDate = endOfWeek(now, { weekStartsOn: 1 });
      } else {
        startDate = startOfMonth(now);
        endDate = endOfMonth(now);
      }

      // Fetch training sessions with attendance data
      const { data: sessions, error } = await supabase
        .from('training_sessions')
        .select(`
          id,
          date,
          start_time,
          end_time,
          training_type,
          name,
          max_participants,
          training_attendance(
            id,
            attended,
            athlete_id
          )
        `)
        .gte('date', format(startDate, 'yyyy-MM-dd'))
        .lte('date', format(endDate, 'yyyy-MM-dd'))
        .order('date', { ascending: true })
        .order('start_time', { ascending: true });

      if (error) throw error;

      return sessions as (TrainingSession & { 
        training_attendance: Array<{ id: string; attended: boolean; athlete_id: string }> 
      })[];
    },
  });

  // Process data into heatmap format
  const heatmapData = React.useMemo(() => {
    if (!data) return [];

    // Initialize heatmap grid
    const days = ['Lun', 'Mar', 'Mié', 'Jue', 'Vie', 'Sáb', 'Dom'];
    const hours = Array.from({ length: 16 }, (_, i) => i + 6); // 6 AM to 9 PM
    const heatmap: HeatmapData[] = [];

    // Initialize all slots to 0
    days.forEach((day, dayIndex) => {
      hours.forEach(hour => {
        heatmap.push({
          day,
          dayIndex,
          hour,
          intensity: 0,
          sessionCount: 0,
          totalAttendance: 0,
          averageAttendance: 0,
        });
      });
    });

    // Process each session
    data.forEach(session => {
      const sessionDate = parseISO(session.date);
      const dayOfWeek = sessionDate.getDay();
      const adjustedDayIndex = dayOfWeek === 0 ? 6 : dayOfWeek - 1; // Convert to Monday=0 format
      const day = days[adjustedDayIndex];
      
      const startHour = parseInt(session.start_time.split(':')[0]);
      const endHour = parseInt(session.end_time.split(':')[0]);
      
      // Count attendance
      const attendanceCount = session.training_attendance?.filter(a => a.attended).length || 0;
      const totalRegistered = session.training_attendance?.length || 0;
      
      // Update heatmap for each hour the session spans
      for (let hour = startHour; hour <= Math.min(endHour, 21); hour++) {
        const heatmapIndex = heatmap.findIndex(h => h.day === day && h.hour === hour);
        if (heatmapIndex !== -1) {
          heatmap[heatmapIndex].sessionCount += 1;
          heatmap[heatmapIndex].totalAttendance += attendanceCount;
        }
      }
    });

    // Calculate intensity and average attendance
    heatmap.forEach(cell => {
      if (cell.sessionCount > 0) {
        cell.averageAttendance = Math.round(cell.totalAttendance / cell.sessionCount);
        
        // Calculate intensity based on session count and attendance
        // More sessions = higher base intensity, more attendance = intensity multiplier
        const sessionIntensity = Math.min(cell.sessionCount * 25, 75); // Up to 75% for session count
        const attendanceBonus = Math.min(cell.averageAttendance * 2, 25); // Up to 25% bonus for attendance
        cell.intensity = Math.min(sessionIntensity + attendanceBonus, 100);
      }
    });

    return heatmap;
  }, [data]);

  // Calculate statistics
  const statistics = React.useMemo(() => {
    if (!data || data.length === 0) {
      return {
        totalSessions: 0,
        totalAttendance: 0,
        averageAttendance: 0,
        peakHour: 'N/A',
        peakDay: 'N/A',
        busiest: { day: 'N/A', hour: 'N/A', count: 0 }
      };
    }

    const totalSessions = data.length;
    const totalAttendance = data.reduce((sum, session) => 
      sum + (session.training_attendance?.filter(a => a.attended).length || 0), 0
    );
    const averageAttendance = totalSessions > 0 ? Math.round(totalAttendance / totalSessions) : 0;

    // Find peak times
    const hourCounts: { [hour: number]: number } = {};
    const dayCounts: { [day: string]: number } = {};
    const cellCounts: { [key: string]: number } = {};

    heatmapData.forEach(cell => {
      if (cell.sessionCount > 0) {
        hourCounts[cell.hour] = (hourCounts[cell.hour] || 0) + cell.sessionCount;
        dayCounts[cell.day] = (dayCounts[cell.day] || 0) + cell.sessionCount;
        cellCounts[`${cell.day}-${cell.hour}`] = cell.sessionCount;
      }
    });

    const peakHour = Object.entries(hourCounts).reduce((max, [hour, count]) => 
      count > max.count ? { hour: parseInt(hour), count } : max, { hour: 0, count: 0 }
    );

    const peakDay = Object.entries(dayCounts).reduce((max, [day, count]) => 
      count > max.count ? { day, count } : max, { day: 'N/A', count: 0 }
    );

    const busiest = Object.entries(cellCounts).reduce((max, [key, count]) => {
      const [day, hour] = key.split('-');
      return count > max.count ? { day, hour: parseInt(hour), count } : max;
    }, { day: 'N/A', hour: 0, count: 0 });

    return {
      totalSessions,
      totalAttendance,
      averageAttendance,
      peakHour: peakHour.hour > 0 ? `${peakHour.hour}:00` : 'N/A',
      peakDay: peakDay.day,
      busiest
    };
  }, [data, heatmapData]);

  return {
    heatmapData,
    statistics,
    isLoading,
    error,
    rawData: data || []
  };
};