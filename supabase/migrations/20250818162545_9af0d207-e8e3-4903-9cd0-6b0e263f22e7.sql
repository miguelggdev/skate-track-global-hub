...
DO $$ BEGIN
    CREATE TYPE public.app_role AS ENUM ('admin', 'coach', 'athlete', 'delegate', 'leader', 'finance');
EXCEPTION
...
-- 5. Create security definer function to get user role (prevents recursion)
...
-- 6. Migrate existing roles from profiles to user_roles table
...
-- 7. Update profiles table to remove role column and simplify RLS
ALTER TABLE public.profiles DROP COLUMN IF EXISTS role;

-- 8. Drop existing problematic policies on profiles
...
-- 9. Create simple, non-recursive policies for profiles
...
-- 10. Create policies for user_roles table
...
-- 11. Update other table policies to use the new role system
-- Athletes table
...
-- 12. Update club_settings policies
...
-- 13. Update handle_new_user function to use user_roles
...
BEGIN
  ...
  user_role_value := COALESCE((NEW.raw_user_meta_data ->> 'role')::app_role, 'athlete'::app_role);
