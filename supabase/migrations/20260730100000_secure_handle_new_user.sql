-- Migration: 20260730100000_secure_handle_new_user.sql
-- Patches handle_new_user to:
-- 1. Auto-assign 'admin' to the first registered user
-- 2. Block self-assignment of 'admin' via signup metadata (security fix)
...
BEGIN
  -- Security: determine role based on whether any admin already exists
  SELECT COUNT(*) INTO v_admin_count
  FROM public.user_roles
  WHERE role = 'admin';

  IF v_admin_count = 0 THEN
    -- First registered user in the system becomes admin
    v_role := 'admin';
  ELSE
    -- Subsequent users: honour metadata role only for non-admin values
    v_role := CASE
      WHEN (NEW.raw_user_meta_data ->> 'role') IN ('leader', 'coach', 'delegate', 'finance', 'athlete')
