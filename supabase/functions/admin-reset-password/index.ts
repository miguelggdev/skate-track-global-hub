...
async function checkRateLimit(supabase: ReturnType<typeof createClient>, ip: string): Promise<boolean> {
  const key = `admin-reset-password:${ip}`;
  const windowMs = 300;
  const maxRequests = 5;
...
serve(async (req) => {
  ...
  try {
    ...
    if (roleError || !isAdmin) {
      return json({ error: 'Solo los administradores pueden restablecer contraseñas' }, 403);
    }
    ...
    if (!/[a-zA-Z]/.test(newPassword) || !/[0-9]/.test(newPassword)) {
      return json({ error: 'La contraseña debe contener letras y números' }, 400);
    }
    ...
    return json({ success: true, message: 'Contraseña actualizada exitosamente' });
  } catch (error) {
